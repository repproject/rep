...
WHERE LD.LV_CD = '2'
AND (LD.LEGL_DONG_CD LIKE '11%' OR LD.LEGL_DONG_CD LIKE '41%') #경기도
AND LD.USE_YN = 'Y'
ORDER BY PYNG_PER_SALE_PRC DESC 
;
SELECT *
...
WHERE NOT EXISTS (
    ...
    WHERE LD2.LV_CD = '2'
    AND LD.LEGL_DONG_CD = CONCAT(SUBSTRING(LD2.LEGL_DONG_CD,1,4),'000000')
) #성남시 용인시등 LV2에 시와 구가 같이 있는 경우 시를 제외 한다.
AND LD.LV_CD = '2'
AND (LD.LEGL_DONG_CD LIKE '11%' OR LD.LEGL_DONG_CD LIKE '41%')
#AND LEGL_DONG_WHL_NM LIKE '%성남%'
;

SELECT * 
FROM KRED_LEGL_DONG LD
   , KRED_LEGL_DONG LD2
WHERE LD.LEGL_DONG_CD = (+) LD2.UP_LEGL_DONG_CD
AND LD.LV_CD = '2'
AND LD.USE_YN = 'N'
