...
#Lv3
class CrawlingMultiGO(REP_MIG.CrawlingBasicMulti):
    siteCode = 'GO'
    sleepStamp = dicSiteBasic['GO']['SLEP_TIME']

    #[LV3 구현]각 Lv3 Class(웹사이트(url) 별로) URL을 만드는 부분을 정의
    def selfMakeURL(self,dicStrdData = None):
        url = REP_URL.URLMaker("APTTrade")
...
#Lv4 아파트매매 실거래 상세 자료
class CrawlingMultiGOAptTrade(CrawlingMultiGO):
    funcName = "CrawlingMultiGOAptTrade"
    fetchSqlId =  "selectDupLeglLv2Dong"
